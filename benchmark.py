...
def plot_recall_at_k(recall_dict, title="Recall@K Curve"):
    plt.figure(figsize=(10, 6))
    for method, recalls in recall_dict.items():
        plt.plot(range(1, len(recalls) + 1), recalls, marker='o', label=method)
    plt.xlabel("K")
    plt.ylabel("Recall@K")
    plt.title(title)
    plt.grid(True)
...
def run_lightweight_benchmark(base_path, query_path, k=10, dim=96):
    ...
    for index_type in ["flat", "ivf", "hnsw", "lsh"]:
        ...
        recall_curve = evaluate_results(true_D, I, k)
        recall_at_k = recall_curve[k - 1]
        precision = evaluate_precision(true_D, I)
...
if __name__ == "__main__":
    ...
    run_lightweight_benchmark(base_path, query_path, k=10, dim=96)